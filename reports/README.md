...
Our continuous integration setup mainly runs on GitHub. The workflow files are organised into 5 files in the `.github/workflows` folder. A link to them on our main branch can be found [here](https://github.com/martin5709/Group83-MLOps-02476/tree/main/.github/workflows). It consists of several checks, such as unit testing, linting, checking that there is no dependency conflict between different requirements files, checks that people haven't added large files, and adding code coverage badges. There is also a hidden implicit CI step, which is that whenever there is a push to main, the build job for the training docker container and the api docker container is automatically run in Cloud Build. An example of a triggered workflow can be seen [here](https://github.com/martin5709/Group83-MLOps-02476/actions/runs/12951371828). We make sure to test on multiple python versions (3.11 and 3.12) and several operating systems (`windows-latest` and `ubuntu-latest`), though while the tests pass, the windows tests do not truly work, since they cannot parse the `CNNDetection` pickle properly. Caching is used in these tests, as can be seen by the usage of `cache` and `cache-dependency-path` for `pip` in the `steps` part of the `test.yaml`. We additionally have the policy of only permitting pull requests to `main`, never direct pushes, to make sure that accidental pushes to `main` don't happen. Likewise, to merge a pull request into `main`, the PR must pass the Python 3.11 `ubuntu-latest` unit testing, and the check for large files and the python requirements check.
...
2. `check_file_size.yaml` -- Utilises piping and bash commands to find the largest file and checks this is below 8 MB (to prevent people from accidentally submitting large files to `main`).

3. `check_python_requirements.yaml` -- Compares the two Python requirement files. If any line overlaps between the two, it errors.
