...
def test_data():
    ...
    assert abs(torch.Tensor.mean(main_dataset)) <= 0.05 , f'Expected the data to be centered; however, the mean is {torch.Tensor.mean(main_dataset)}'
    assert  abs(1 - torch.std(main_dataset)) <= 0.05 , f'Expected the data to be normalised; however, the std is {torch.std(main_dataset)}'
